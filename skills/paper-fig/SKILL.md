---
name: paper-fig
description: Create editable paper figures, method pipelines, and architecture diagrams as PPTX, faithfully following the user's template or design. Use supplied references or generate full-page previews, suggest local refinements, and crop or generate high-resolution assets for editable reconstruction.
---

# Paper Fig

Create academic figures with the presentation workflow below. Default to a single editable PPTX page unless the user specifies another page count or output. Keep scientific labels, equations, modules, and connections editable where practical; use raster assets for the imagery that needs them. This is an independent adaptation of Presentations.

Use this skill in an environment that provides the presentation runtime, including `@oai/artifact-tool`, and an image-generation tool when generation is needed. Resolve runtime paths through `load_workspace_dependencies` when available, or use compatible runtime paths already supplied by the host. Export the resolved `RUNTIME_NODE`, `RUNTIME_NODE_MODULES`, `RUNTIME_BIN_DIR`, and `RUNTIME_PYTHON` variables to the processes that need them. Set `SKILL_DIR` to this installed skill folder. When the loader is unavailable, compatible runtime paths already supplied by the host satisfy the runtime-discovery instructions in the inherited references, including any bundled LibreOffice path needed for rendering. The ZIP contains the skill resources, not the external runtime or image-generation service; if a required capability is missing, state which capability is unavailable.
...
When the user asks to match or reconstruct a template or reference, reproduce its appearance faithfully. Read and follow [template following](references/template_following.md). Preserve the canvas dimensions, layout, positions, proportions, fonts, colors, spacing, imagery, labels, and connection routes unless the user's task calls for changes. The user's wording, scientific notation, and design take precedence over general writing advice and default styling below. For the selected reference, whether supplied or generated, those defaults do not authorize rewriting, adding, removing, or rearranging reference content; scientific content remains grounded in the user's information. Editability is an implementation requirement, not permission to redesign. Limit unrequested changes to small technical fixes needed to reproduce the reference correctly, such as preventing conversion-induced clipping; do not reinterpret its visual style. A reference supplied only for content or inspiration has the role the user assigns to it. When generating a preview, follow the user's design directions; once selected, treat the preview as the design to reconstruct, not an invitation to design the page again.

## Artifact Template Selection

For academic figures, use the supplied reference or the full-page preview workflow below to establish the design; do not open a presentation template picker by default. Read [template selection](references/template-elicitation.md) only when the user explicitly asks to browse or select presentation templates.
...
3. **Generate a full-page design preview.** If the user supplies a reference for reconstruction, use it directly without generating another preview. Otherwise, after organizing the page content, scientific structure, and overall layout according to the user's design directions, read [drawing prompt guidance](references/preview_prompt.md). Write a complete, ready-to-use image-generation prompt covering the canvas, layout and region proportions, typography, palette, connections, and concrete image assets in each region. Include meaningful image assets by default unless the user requests an image-free design, and describe their subjects, counts, viewpoints, placement, and consistency across related states. Save the finished prompt as `work/preview-prompt.md` in the task workspace, then use it to generate the full-page design reference. The preview may contain text, titles, equations, arrows, and diagrams to communicate the entire design. Scientific content and connections remain grounded in the information supplied by the user.
4. **Suggest local preview refinements.** After generating a full-page preview, briefly look for visibly unbalanced empty areas, crowding, misalignment, or overlaps. When a material issue is present, offer up to three concrete local suggestions naming the affected area and adjustment; otherwise continue. Preserve useful whitespace and scientific content rather than adding elements just to fill space. Apply a repair only when requested or already authorized, limiting it to the affected area while preserving the rest of the design. Otherwise continue with the unmodified preview and leave the suggestions optional. Skip this step for supplied or already selected references unless feedback or repair is requested. This short design pass adds no separate report, repeated review loop, or mandatory approval pause.
5. **Faithfully reconstruct the reference and obtain high-resolution assets.** Follow [template following](references/template_following.md) for inspecting the reference, rebuilding editable objects, and comparing appearance, together with the fidelity rule above. Match the selected reference closely; apply design changes only within the scope requested by the user. During construction, crop visual assets directly from the selected reference (supplied image or generated preview) when its resolution is sufficient. When new assets are needed, use that selected full-page reference as an image reference and specify each asset's content, aspect ratio, and resolution needs while preserving its appearance, viewpoint, and visual style. Independent assets may be generated in parallel in batches of at most four, sharing the same reference and style direction; use sequential calls if the available tool requires them. Continue with native editable construction and the existing export workflow. No additional approval checkpoint or separate checklist is required by these preview and asset steps.
...
7. Export a draft PPTX in the private build directory, then follow [validation and delivery](references/finalization.md). Inspect the rendered slides and fix problems. Do not change the user's design just to satisfy an automated style suggestion.
